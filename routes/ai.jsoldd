...
if (intent === "CUSTOMER_FOLLOW_UP") {
    ...
    // Deterministic calculation: rank products locally.
    // This avoids sending the complete product dataset to Groq.
    const products = Array.isArray(aiData?.products)
        ? aiData.products
        : [];

    if (!products.length) {
        answer = "No product performance data is available.";
        aiData.productPerformance = {
            highestProduct: null,
            ranking: []
        };
    } else {

        const getPerformanceValue = (p) => {
            const candidates = [
                p.performancePercent,
                p.performance,
                p.growthPercent,
                p.currentAmount,
                p.currentQty
        ...
        const ranking = products
            .map(p => ({
                productName:
                    ...
                    p.ProductName ||
                    "Unknown Product",
                performanceValue: getPerformanceValue(p),
                performancePercent:
                    p.performancePercent ??
                    p.performance ??
                    p.growthPercent ??
            ...
            .sort((a, b) =>
                Number(b.performanceValue) -
                Number(a.performanceValue)
            );

        const top = ranking[0];

        answer =
            `**Product with the highest performance:**\n\n` +
            `- **Product:** ${top.productName}\n` +
            `- **Performance:** ${
                top.performancePercent !== null
                    ? Number(top.performancePercent).toLocaleString("en-IN", {
                        maximumFractionDigits: 2
                    }) + "%"
                    : Number(top.performanceValue).toLocaleString("en-IN", {
                        maximumFractionDigits: 2
                    })
            }`;
...
const productPerformance =
    ...
    q.includes("which product has the highest performance") ||
    q.includes("which product has the best performance") ||
    q.includes("best performing product") ||
    q.includes("highest performing product");
            ...
            sets.find(set =>
                ...
                set.some(row =>
                    ...
                    (
                        ...
                        "productName" in row ||
                        "Product" in row ||
                        "GrowthPercent" in row ||
                        "PerformancePercent" in row ||
                        "Performance" in row ||
                        "CurrentAmount" in row ||
                        "CurrentQty" in row
                    )
                )
            ) || [];
